...
**フェーズタイプ:**
- `single`: 1枚のカードを選択
- `combo`: 2枚のカードを選択（シナジーボーナス最大10点）
- `triple`: 3枚のカードを選択（シナジーボーナス最大15点、hardシナリオの最終フェーズで使用）

### ハードシナリオのトリプルコンボ

以下のhardシナリオでは、最終フェーズ（フェーズ5）でトリプルコンボ（3枚選択）を使用:

- **AI反乱鎮圧作戦** (`ai_rebellion`)
- **魔王討伐の旅** (`demon_lord`)
...
| レジェンドカード | レジェンド使用 | +30 |
| 完璧適合 | 適合度95以上 | +25 |
| コンボシナジー | 2枚選択時、良い組み合わせ | +0〜10 |
| トリプルシナジー | 3枚選択時、良い組み合わせ | +0〜15 |
